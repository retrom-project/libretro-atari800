...
work=$(mktemp -d "$root/.retrom-build/retrom-atari800-web.XXXXXX")
...
if ! docker run --rm --platform linux/amd64 --hostname retrom-atari800 \
  ...
  /recipe/build-emulatorjs-core.sh atari800 \
...
stage="$work/stage"
mkdir -p "$stage"
install -m 0644 "$work/raw/atari800_libretro.js" "$stage/"
install -m 0644 "$work/raw/atari800_libretro.wasm" "$stage/"
cat "$root/atari800/COPYING" "$work/raw/retroarch-COPYING" > "$stage/license.txt"
printf '%s\n' '{"minimumEJSVersion":"4.2.2","version":"1.18"}' > "$stage/build.json"
printf '%s\n' '{"name":"atari800","extensions":["atr","atx","cas","car","rom","bin","xex"],"makeoptions":{"buildpath":"./","makescript":"Makefile","arguments":[]},"options":{},"save":true,"license":"LICENSE","repo":"https://github.com/retrom-project/libretro-atari800"}' > "$stage/core.json"
chmod 0644 "$stage/build.json" "$stage/core.json" "$stage/license.txt"

(cd "$stage" && 7z a -mtm=off -mta=off -mtc=off -bd -bso0 -bsp0 -t7z "$output/atari800-wasm.data" \
  atari800_libretro.js atari800_libretro.wasm build.json core.json license.txt)
